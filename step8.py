        ...
        if use_default_bins:
            try:
                with st.spinner("🚀 正在从云端加载默认坐标轴刻度表，请稍候..."):
                    ...
                    default_bins_url = "https://raw.githubusercontent.com/minamao0825/annual-report-assets/main/%E5%9D%90%E6%A0%87%E8%BD%B4%E5%88%BB%E5%BA%A6%E8%A1%A8-step8%20%E7%9A%84%E5%89%AF%E6%9C%AC.xlsx"
        ...
        if use_default:
            try:
                with st.spinner("🚀 正在从云端加载默认注释表，请稍候..."):
                    # 🌟 核心修改 2：替换为 GitHub 文件的原始读取链接
                    default_url = "https://raw.githubusercontent.com/minamao0825/annual-report-assets/main/RD-%E5%9B%BE%E7%89%87%E5%86%85%E5%AE%B9%E5%88%86%E6%9E%90%E5%92%8C%E6%B3%A8%E9%87%8A%E6%A8%A1%E6%9D%BF-step8.xlsx"
                    import requests, io
...
# 💡 友情提示：记得在后续代码中，使用 df_notes 之前，先判断一下 if df_notes is not None:
    ...
    def render_pure_chart_entity(m_id, print_mode):
        ...
        # 保险服务费用构成
            ...
            if avg:
                # 先把表头和单元格的 HTML 拼接好（用单引号代替双引号转义）
                th_elements = ''.join([f"<th style='padding:6px 8px;background-color:{colors[f]};color:white;border:1px solid white;'>{display[f]}</th>" for f in fields])
                td_elements = ''.join([f"<td style='padding:6px 8px;text-align:center;border:1px solid #EAEAEA;'>{avg[f]:.1f}%</td>" for f in fields])
                
                # 然后再把它们放进主 HTML 字符串中
                html = f"<table style='width:100%;border-collapse:collapse;font-family:sans-serif;font-size:11px;margin-bottom:15px;'><tr style='background-color:#00338D;color:white;text-align:center;font-weight:bold;'><th style='padding:6px 8px;border:1px solid white;'>项目</th>{th_elements}</tr><tr style='background-color:#F8F9FA;'><td style='padding:6px 8px;font-weight:bold;border:1px solid #EAEAEA;'>各公司平均占比（样本均值）</td>{td_elements}</tr></table>"
                st.markdown(html, unsafe_allow_html=True)
